...
function Config() {
  ...
  const [connectionTypeInput, setConnectionTypeInput] =
      ...
      const fetchTables = async () => {
        try {
          const api = vg.createAPIContext({
            coordinator: new vg.Coordinator(
              vg.socketConnector(connectionURLInput as any),
            ),
          });
          ...
          const formattedResult = result
            ?.toArray()
            ?.map((tableItem: { table_name: string }) => tableItem.table_name)
            .filter((name: string) => !name.includes('_expr'));
        ...
        <div className='card mt-4 bg-body-tertiary shadow-sm'>
          <div className='card-body'>
            ...
            {connectionTypeInput === 'wasm' ? (
              ...
              >
                {wasmTables.map((wasmTable: string, index: number) => (
                  <option value={wasmTable} key={index}>
                    {wasmTable}
                  </option>
                ))}
              ...
              >
                ...
                {serverTables.map((serverTable: string, index: number) => (
                  <option value={serverTable} key={index}>
                    {serverTable}
                  </option>
                ))}
              </select>
            ...
            <div className='d-flex align-items-end mt-4'>
              ...
              <span className='text-ss text-muted fst-italic ms-auto'>
                Want to use your own data? Follow the instructions{' '}
                <a
                ...
                to run your own local WebSocket server.
              </span>
            </div>
          </div>
        </div>
